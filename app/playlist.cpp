...
void Playlist::importPlaylist() {
    ...
    if (path != nullptr) {
        ...
        int added = m_db.addToPlaylists(path, m_user);
        if (added > 0) {
            MyTreeWidgetItem *my = m_db.addPlaylist(ui->treeWidget, path, added, m_main);
            while (!line.isNull()) {
                int added_track = m_db.addToTracks(line);
                if (added_track > 0) {
                    m_db.addToTrackPlaylists(added_track, added);
                    m_db.addTrack(line, my, m_main);
                }
                line = in.readLine();
            }
        }
    ...
    if (ui->treeWidget->currentItem()->parent() != nullptr) {
        ...
        myMenu.addAction("add new playlist", this, [this]() {
            ...
            if (caseInput != nullptr) {
                int check = m_db.addToPlaylists(caseInput, m_user);
                if (check != -1)
        ...
        myMenu.addAction("delete playlist " + ui->treeWidget->currentItem()->text(0), this, [this, point]() {
            ...
            if (ui->treeWidget->topLevelItemCount() == 0) {
                if (m_db.addToPlaylists("New Playlist", m_user) != -1)
...
Playlist::Playlist(QWidget *parent) : QWidget(parent),
    ...
    m_db.createDataBase();
    m_db.SignUp("users", "1234");
    m_user = "users";
    if (ui->treeWidget->topLevelItemCount() == 0)
        m_db.addToPlaylists("New Playlist " + m_user, m_user);
    m_db.selectPlaylistFromDataBase(ui->treeWidget, m_user, m_main);
